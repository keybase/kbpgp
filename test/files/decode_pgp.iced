...
exports.public_keys_now = (T,cb) ->
  names = [ "weston" ]
  # Set a date in the past, when none of these keys had expired.
  opts = { now : (new Date(2016, 2, 22, 0, 0, 0)).getTime()/1000 }
  for n in names
    await KeyManager.import_from_armored_pgp { raw : keys[n], opts } , defer err, km, warnings
    T.no_error err
    if err?
...
exports.decode_pgp_secret_key_1 = (T,cb) ->
  ...
  passphrase = "catsdogs"
  [err, packets] = parse (Buffer.from skey, 'base64')
  T.no_error err
  T.waypoint "parsed"
...
exports.public_key_expired_uid = (T,cb) ->
  ...
  T.equal err.message, "no valid primary key self-signature or key(s) have expired", "the right error"
  cb()
...
expected_pgp_body = Buffer.from expected_pgp_payload, "base64"
...
expected_nacl_body = Buffer.from expected_nacl_payload, "base64"
