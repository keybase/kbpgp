...
exports.encrypt = (T,cb) ->
  await kb.box { encrypt_for : km, msg : plaintext }, T.esc(defer(tmp), cb)
  ctext = tmp
  cb()

exports.decrypt_1 = (T, cb) ->
  await kb.unbox { encrypt_for : km, armored : ctext}, T.esc(defer(tmp), cb)
...
exports.enc_export_private_key = (T,cb) ->
  await km.export_private {}, T.esc(defer(tmp), cb)
  priv_raw = tmp
  cb()

exports.enc_import_private_key = (T,cb) ->
  await kb.EncKeyManager.import_private { raw : priv_raw }, T.esc(defer(tmp), cb)
  km2 = tmp
  cb()
...
exports.sign_1 = (T,cb) ->
  await kb.box { sign_with : km, msg : plaintext }, T.esc(defer(tmp), cb)
  ctext = tmp
  cb()

exports.verify_1 = (T, cb) ->
  await kb.unbox { armored : ctext}, T.esc(defer(tmp), cb)
...
exports.sig_import_private_key = (T,cb) ->
  await kb.KeyManager.import_private { raw : priv_raw }, T.esc(defer(tmp), cb)
  km2 = tmp
  cb()

exports.sign_2 = (T,cb) ->
  await kb.box { sign_with : km2, msg : plaintext }, T.esc(defer(tmp), cb)
  ctext = tmp
  cb()
