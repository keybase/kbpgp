      ...
      ret = (function() {
        switch (this.type) {
          ...
          case C.s2k.salt_iter:
            ...
            console.warn("pw -> " + (passphrase.toString('utf8')) + " ; salt -> " + (this.salt.toString('hex')));
            if ((numBytes != null) && (numBytes === 24 || numBytes === 32)) {
              key = this.hash(isp);
              Buffer.concat([key, this.hash(Buffer.concat([new Buffer([0]), isp]))]);
              return console.warn("in numBytes 24,32; nb = " + numBytes);
            } else {
