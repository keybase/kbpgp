...
(function() {
  var AES, Base, Decryptor, Encryptor, SlicerBuffer, WordArray, decrypt, encrypt, iced, make_esc, repeat, rng, test, triplesec, xbt, xxd, __iced_k, __iced_k_noop,
  ...
  Base = (function(_super) {
    ...
    function Base(_arg) {
      ...
      this.c_block_size = this.cipher.blockSize;
      block_size = this.c_block_size * 1024 * 8;
      this.out_bufs = [];
  ...
  Encryptor = (function(_super) {
    ...
    Encryptor.prototype.xbt_type = function() {
      return "Encryptor";
    };
    ...
    Encryptor.prototype._emit_buf = function(buf) {
      var wa;
      wa = WordArray.from_buffer(buf.slice(0, this.c_block_size));
      wa.xor(this.FRE, {
    ...
    Encryptor.prototype._init_iv = function(prefixrandom) {
      ...
      this.FR = new Buffer((function() {
        ...
        _results = [];
        for (i = _i = 0, _ref = this.c_block_size; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          _results.push(0);
        }
        return _results;
      }).call(this));
      ...
      canary = new Buffer((function() {
        ...
        for (i = _i = 0; _i < 2; i = ++_i) {
          _results.push(b.readUInt8(i) ^ prefixrandom.readUInt8(this.c_block_size + i));
        }
        return _results;
      }).call(this));
      ...
      ct = this.compact();
      ct.copy(this.FR, 0, offset, offset + this.c_block_size);
      return this._enc();
    };
    ...
    Encryptor.prototype._pad = function(_arg) {
      var a, b, data, eof, err, rem;
      data = _arg.data, eof = _arg.eof;
      err = null;
      if ((a = data.length) % (b = this.c_block_size) === 0) {
      ...
      if (rem === 0) {
        ...
        data = Buffer.concat([
          data, new Buffer((function() {
            var _i, _ref, _results;
            _results = [];
            for (_i = 0, _ref = b - rem; 0 <= _ref ? _i < _ref : _i > _ref; 0 <= _ref ? _i++ : _i--) {
              _results.push(0);
            }
            return _results;
          })())
    ...
    Encryptor.prototype._v_inblock_chunk = function(_arg, cb) {
      ...
      if (err == null) {
        if (this._first) {
          this._first = false;
          this._do_first(data.slice(0, this.c_block_size));
          data = data.slice(this.c_block_size);
        }
    ...
    Encryptor.prototype._do_first = function(data, cb) {
      ...
      if (this.resync) {
        ...
        this.out_bufs.push(buf);
        ct = this.compact();
        return ct.copy(this.FR, 0, ct.length - this.c_block_size, ct.length);
      }
    };
    ...
    Encryptor.prototype._do_block = function(data, cb) {
      ...
      _results = [];
      for (i = _i = 0, _ref = data.length, _ref1 = this.c_block_size; _ref1 > 0 ? _i < _ref : _i > _ref; i = _i += _ref1) {
        this._enc();
        _results.push(this._emit_buf(data.slice(i, i + this.c_block_size)));
      }
      return _results;
    };
    ...
    Encryptor.prototype._flush_and_trunc = function(eof) {
      ...
      err = null;
      if (eof) {
        n_wanted = this._input_len + this.c_block_size + 2;
  ...
  Decryptor = (function(_super) {
    ...
    Decryptor.prototype.next_block = function() {
      return WordArray.from_buffer(this.sb.read_buffer_at_most(this.c_block_size));
    };
    ...
    Decryptor.prototype.check = function() {
      ...
      iblock = new WordArray((function() {
        var _i, _ref, _results;
        _results = [];
        for (i = _i = 0, _ref = this.c_block_size / 4; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          _results.push(0);
        }
        return _results;
      }).call(this));
