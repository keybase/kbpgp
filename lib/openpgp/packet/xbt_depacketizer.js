// Generated by IcedCoffeeScript 1.7.1-c
(function() {
  var C, Depacketizer, PacketParser, PgpReadBufferer, bufcat, iced, make_esc, xbt, __iced_k, __iced_k_noop,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  iced = require('iced-runtime').iced;
  __iced_k = __iced_k_noop = function() {};

  xbt = require('../../xbt');

  bufcat = require('../../util').bufcat;

  C = require('../../const').openpgp;

  make_esc = require('iced-error').make_esc;

  PgpReadBufferer = (function(_super) {
    __extends(PgpReadBufferer, _super);

    function PgpReadBufferer() {
      return PgpReadBufferer.__super__.constructor.apply(this, arguments);
    }

    PgpReadBufferer.prototype._read_uint8 = function(cb) {
      var buf, err, out, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PgpReadBufferer._read_uint8"
          });
          _this._read({
            exactly: 1
          }, __iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                err = arguments[0];
                return buf = arguments[1];
              };
            })(),
            lineno: 13
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          out = typeof err !== "undefined" && err !== null ? null : buf.readUInt8(0);
          return cb(err, out);
        };
      })(this));
    };

    PgpReadBufferer.prototype._read_uint16 = function(cb) {
      var buf, err, out, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PgpReadBufferer._read_uint16"
          });
          _this._read({
            exactly: 2
          }, __iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                err = arguments[0];
                return buf = arguments[1];
              };
            })(),
            lineno: 20
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          out = typeof err !== "undefined" && err !== null ? null : buf.readUInt16BE(0);
          return cb(err, out);
        };
      })(this));
    };

    PgpReadBufferer.prototype._read_uint32 = function(cb) {
      var buf, err, out, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PgpReadBufferer._read_uint32"
          });
          _this._read({
            exactly: 4
          }, __iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                err = arguments[0];
                return buf = arguments[1];
              };
            })(),
            lineno: 27
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          out = typeof err !== "undefined" && err !== null ? null : buf.readUInt32BE(0);
          return cb(err, out);
        };
      })(this));
    };

    PgpReadBufferer.prototype._read_string = function(cb) {
      var buf, esc, len, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      esc = make_esc(cb, "_read_string");
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PgpReadBufferer._read_string"
          });
          _this._read_uint8(esc(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                return len = arguments[0];
              };
            })(),
            lineno: 35
          })));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          (function(__iced_k) {
            __iced_deferrals = new iced.Deferrals(__iced_k, {
              parent: ___iced_passed_deferral,
              filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
              funcname: "PgpReadBufferer._read_string"
            });
            _this._read({
              exactly: len
            }, esc(__iced_deferrals.defer({
              assign_fn: (function() {
                return function() {
                  return buf = arguments[0];
                };
              })(),
              lineno: 36
            })));
            __iced_deferrals._fulfill();
          })(function() {
            return cb(null, buf);
          });
        };
      })(this));
    };

    return PgpReadBufferer;

  })(xbt.ReadBufferer);

  exports.Depacketizer = Depacketizer = (function(_super) {
    __extends(Depacketizer, _super);

    function Depacketizer(_arg) {
      this.packet_version = _arg.packet_version, this.packet_xbt = _arg.packet_xbt;
      Depacketizer.__super__.constructor.call(this, {});
      this._total = 0;
      this._flow_rem = 0;
      this.packet_xbt.set_parent(this);
    }

    Depacketizer.prototype._depacketize_1 = function(cb) {
      var esc, final, len, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      esc = make_esc(cb, "_depacketize_1");
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "Depacketizer._depacketize_1"
          });
          _this._find_length(esc(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                final = arguments[0];
                return len = arguments[1];
              };
            })(),
            lineno: 53
          })));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          (function(__iced_k) {
            __iced_deferrals = new iced.Deferrals(__iced_k, {
              parent: ___iced_passed_deferral,
              filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
              funcname: "Depacketizer._depacketize_1"
            });
            _this._stream_packet(final, len, esc(__iced_deferrals.defer({
              lineno: 54
            })));
            __iced_deferrals._fulfill();
          })(function() {
            _this._total += len;
            return cb(null, final);
          });
        };
      })(this));
    };

    Depacketizer.prototype._find_length = function(cb) {
      var err, esc, final, first, nxt, ret, second, tag, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      esc = make_esc(cb, "_find_length");
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "Depacketizer._find_length"
          });
          _this._read_uint8(esc(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                return tag = arguments[0];
              };
            })(),
            lineno: 62
          })));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          err = ret = null;
          final = true;
          (function(__iced_k) {
            if (_this.packet_version === C.packet_version.old) {
              nxt = (function() {
                switch (tag & 0x03) {
                  case 0:
                    return this._read_uint8.bind(this);
                  case 1:
                    return this._read_uint16.bind(this);
                  case 2:
                    return this._read_uint32.bind(this);
                  default:
                    err = new Error("Cannot handle old-style wildcard lengths");
                    return null;
                }
              }).call(_this);
              (function(__iced_k) {
                if (nxt != null) {
                  (function(__iced_k) {
                    __iced_deferrals = new iced.Deferrals(__iced_k, {
                      parent: ___iced_passed_deferral,
                      filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                      funcname: "Depacketizer._find_length"
                    });
                    nxt(esc(__iced_deferrals.defer({
                      assign_fn: (function() {
                        return function() {
                          return ret = arguments[0];
                        };
                      })(),
                      lineno: 74
                    })));
                    __iced_deferrals._fulfill();
                  })(__iced_k);
                } else {
                  return __iced_k();
                }
              })(__iced_k);
            } else {
              (function(__iced_k) {
                __iced_deferrals = new iced.Deferrals(__iced_k, {
                  parent: ___iced_passed_deferral,
                  filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                  funcname: "Depacketizer._find_length"
                });
                _this._read_uint8(esc(__iced_deferrals.defer({
                  assign_fn: (function() {
                    return function() {
                      return first = arguments[0];
                    };
                  })(),
                  lineno: 77
                })));
                __iced_deferrals._fulfill();
              })(function() {
                (function(__iced_k) {
                  if (first < 192) {
                    return __iced_k(ret = first);
                  } else {
                    (function(__iced_k) {
                      if (first === 255) {
                        (function(__iced_k) {
                          __iced_deferrals = new iced.Deferrals(__iced_k, {
                            parent: ___iced_passed_deferral,
                            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                            funcname: "Depacketizer._find_length"
                          });
                          _this._read_uint32(esc(__iced_deferrals.defer({
                            assign_fn: (function() {
                              return function() {
                                return ret = arguments[0];
                              };
                            })(),
                            lineno: 80
                          })));
                          __iced_deferrals._fulfill();
                        })(__iced_k);
                      } else {
                        (function(__iced_k) {
                          if (first < 224) {
                            (function(__iced_k) {
                              __iced_deferrals = new iced.Deferrals(__iced_k, {
                                parent: ___iced_passed_deferral,
                                filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                                funcname: "Depacketizer._find_length"
                              });
                              _this._read_uint8(esc(__iced_deferrals.defer({
                                assign_fn: (function() {
                                  return function() {
                                    return second = arguments[0];
                                  };
                                })(),
                                lineno: 82
                              })));
                              __iced_deferrals._fulfill();
                            })(function() {
                              return __iced_k(ret = ((first - 192) << 8) + (second + 192));
                            });
                          } else {
                            ret = 1 << (first & 0x1f);
                            return __iced_k(final = false);
                          }
                        })(__iced_k);
                      }
                    })(__iced_k);
                  }
                })(__iced_k);
              });
            }
          })(function() {
            return cb(err, final, ret);
          });
        };
      })(this));
    };

    Depacketizer.prototype._depacketize_all = function(cb) {
      var esc, final, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      esc = make_esc(cb, "_depacketize_all");
      final = false;
      (function(_this) {
        return (function(__iced_k) {
          var _results, _while;
          _results = [];
          _while = function(__iced_k) {
            var _break, _continue, _next;
            _break = function() {
              return __iced_k(_results);
            };
            _continue = function() {
              return iced.trampoline(function() {
                return _while(__iced_k);
              });
            };
            _next = function(__iced_next_arg) {
              _results.push(__iced_next_arg);
              return _continue();
            };
            if (!!final) {
              return _break();
            } else {
              (function(__iced_k) {
                __iced_deferrals = new iced.Deferrals(__iced_k, {
                  parent: ___iced_passed_deferral,
                  filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                  funcname: "Depacketizer._depacketize_all"
                });
                _this._depacketize_1(esc(__iced_deferrals.defer({
                  assign_fn: (function() {
                    return function() {
                      return final = arguments[0];
                    };
                  })(),
                  lineno: 95
                })));
                __iced_deferrals._fulfill();
              })(_next);
            }
          };
          _while(__iced_k);
        });
      })(this)((function(_this) {
        return function() {
          return cb(null);
        };
      })(this));
    };

    Depacketizer.prototype._stream_packet = function(final, len, cb) {
      var data, eof, esc, out, rem, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      rem = len;
      esc = make_esc(cb, "_stream_packet");
      (function(_this) {
        return (function(__iced_k) {
          var _results, _while;
          _results = [];
          _while = function(__iced_k) {
            var _break, _continue, _next;
            _break = function() {
              return __iced_k(_results);
            };
            _continue = function() {
              return iced.trampoline(function() {
                return _while(__iced_k);
              });
            };
            _next = function(__iced_next_arg) {
              _results.push(__iced_next_arg);
              return _continue();
            };
            if (!(rem > 0)) {
              return _break();
            } else {
              (function(__iced_k) {
                __iced_deferrals = new iced.Deferrals(__iced_k, {
                  parent: ___iced_passed_deferral,
                  filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                  funcname: "Depacketizer._stream_packet"
                });
                _this._read({
                  max: rem,
                  min: 1
                }, esc(__iced_deferrals.defer({
                  assign_fn: (function() {
                    return function() {
                      return data = arguments[0];
                    };
                  })(),
                  lineno: 104
                })));
                __iced_deferrals._fulfill();
              })(function() {
                rem -= data.length;
                eof = final && (rem === 0);
                (function(__iced_k) {
                  __iced_deferrals = new iced.Deferrals(__iced_k, {
                    parent: ___iced_passed_deferral,
                    filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
                    funcname: "Depacketizer._stream_packet"
                  });
                  _this.packet_xbt.chunk({
                    data: data,
                    eof: eof
                  }, esc(__iced_deferrals.defer({
                    assign_fn: (function() {
                      return function() {
                        return out = arguments[0];
                      };
                    })(),
                    lineno: 107
                  })));
                  __iced_deferrals._fulfill();
                })(function() {
                  return _next(_this._buffer_out_data(out));
                });
              });
            }
          };
          _while(__iced_k);
        });
      })(this)((function(_this) {
        return function() {
          return cb(null);
        };
      })(this));
    };

    Depacketizer.prototype._parse_loop = function(cb) {
      var err, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "Depacketizer._parse_loop"
          });
          _this._depacketize_all(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                return err = arguments[0];
              };
            })(),
            lineno: 114
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          cb(err);
          if (!(_this._err != null) && !_this._eof) {
            return _this.get_parent()._remux({
              indata: _this._flush_in(),
              outdata: _this._flush_out()
            });
          }
        };
      })(this));
    };

    return Depacketizer;

  })(PgpReadBufferer);

  exports.PacketParser = PacketParser = (function(_super) {
    __extends(PacketParser, _super);

    function PacketParser(_arg) {
      this.demux_klass = _arg.demux_klass;
      PacketParser.__super__.constructor.call(this, {});
    }

    PacketParser.prototype._get_next_demux = function() {
      if (this._next_xbt == null) {
        this._next_xbt = new this.demux_klass({});
        this._next_xbt.set_parent(this);
      }
      return this._next_xbt;
    };

    PacketParser.prototype._parse_loop = function(cb) {
      var err, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PacketParser._parse_loop"
          });
          _this._parse_header(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                return err = arguments[0];
              };
            })(),
            lineno: 135
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          _this._switch_to_flow_mode();
          return cb(err);
        };
      })(this));
    };

    PacketParser.prototype._flow_demux = function(_arg, cb) {
      var data, eof, err, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      data = _arg.data, eof = _arg.eof;
      console.log("flow demux");
      console.log(eof);
      console.log(data);
      console.log(data.length);
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            parent: ___iced_passed_deferral,
            filename: "/home/max/src/keybase/kbpgp/src/openpgp/packet/xbt_depacketizer.iced",
            funcname: "PacketParser._flow_demux"
          });
          _this._get_next_demux().chunk({
            data: data,
            eof: eof
          }, __iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
                err = arguments[0];
                return data = arguments[1];
              };
            })(),
            lineno: 144
          }));
          __iced_deferrals._fulfill();
        });
      })(this)((function(_this) {
        return function() {
          return cb(err, data);
        };
      })(this));
    };

    return PacketParser;

  })(PgpReadBufferer);

}).call(this);
