...
(function() {
  var ASP, BaseEccKey, BaseKeyPair, BigInteger, C, K, MRF, Pair, Pub, SRF, bn, bufeq_secure, iced, konst, make_esc, nbits, nbv, __iced_k, __iced_k_noop, _ref, _ref1,
  ...
  Pub = (function(_super) {
    ...
    Pub.prototype.mod = function(x) {
      return x.mod(this.curve.p);
    };

    Pub.prototype.verify = function(_arg, h, cb) {
      var err, hi, p, r, s, u1, u2, v, w;
      r = _arg[0], s = _arg[1];
      err = null;
      ...
      if (r.signum() === 0 || r.compareTo(this.curve.p) > 0) {
        ...
        w = s.modInverse(this.curve.p);
        u1 = this.mod(hi.multiply(w));
        u2 = this.mod(r.multiply(w));
        p = this.curve.G.multiply(u1).add(this.R.multiply(u2));
        v = this.mod(p.affineX);
        console.log({
          r: r,
          s: s,
          w: w,
