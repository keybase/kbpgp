// Generated by IcedCoffeeScript 1.6.3-g
(function() {
  var C, KeyMaterial, MessageParser, PacketParser, Processor, Signature, SlicerBuffer, UserID, err, fs, iced, inspect, out, p, parse, processor, res, util, __iced_deferrals, __iced_k, __iced_k_noop,
    _this = this;

  iced = require('iced-coffee-script/lib/coffee-script/iced').runtime;
  __iced_k = __iced_k_noop = function() {};

  util = require('../util');

  SlicerBuffer = require('./buffer').SlicerBuffer;

  C = require('../const').openpgp;

  KeyMaterial = require('./keymaterial').KeyMaterial;

  Signature = require('./signature').Signature;

  UserID = require('./userid').UserID;

  inspect = require('util').inspect;

  MessageParser = (function() {
    function MessageParser(slice) {
      this.slice = slice;
    }

    MessageParser.prototype.parse = function() {
      var _results;
      _results = [];
      while (this.slice.rem()) {
        _results.push(this.parse_packet());
      }
      return _results;
    };

    MessageParser.prototype.parse_packet = function() {
      return (new PacketParser(this.slice)).parse();
    };

    return MessageParser;

  })();

  PacketParser = (function() {
    function PacketParser(slice) {
      this.slice = slice;
      this.body = null;
      this.real_packet_len = null;
      this.tag = null;
      this.len = null;
      this.header_len = null;
    }

    PacketParser.prototype.parse_header = function() {
      this.parse_tag_and_len();
      this.header_len || (this.header_len = this.slice.offset());
      this.body || (this.body = new SlicerBuffer(this.slice.read_buffer(this.len)));
      this.real_packet_len || (this.real_packet_len = this.len);
      return this.slice.unclamp();
    };

    PacketParser.prototype.parse = function() {
      this.parse_header();
      return this.parse_body();
    };

    PacketParser.prototype.parse_body = function() {
      var packet, pt, sb;
      pt = C.packet_tags;
      sb = this.body;
      packet = (function() {
        switch (this.tag) {
          case pt.secret_key:
          case pt.secret_subkey:
            return KeyMaterial.parse_private_key(sb);
          case pt.public_key:
            return KeyMaterial.parse_public_key(sb);
          case pt.signature:
            return Signature.parse(sb);
          case pt.userid:
            return UserID.parse(sb);
          default:
            throw new Error("Unknown packet tag: " + tag);
        }
      }).call(this);
      packet.set_tag(this.tag);
      packet.set_lengths(this.real_packet_len, this.header_len);
      return packet;
    };

    PacketParser.prototype.parse_tag_and_len = function() {
      var c;
      if (this.slice.len() < 2 || ((c = this.slice.read_uint8()) & 0x80) === 0) {
        throw new Error("This doesn't look like a binary PGP packet");
      }
      if ((c & 0x40) === 0) {
        return this.parse_tag_and_len_old(c);
      } else {
        return this.parse_tag_and_len_new(c);
      }
    };

    PacketParser.prototype.parse_tag_and_len_old = function(c) {
      this.tag = (c & 0x3f) >> 2;
      return this.len = (function() {
        switch (c & 0x03) {
          case 0:
            return this.slice.read_uint8();
          case 1:
            return this.slice.read_uint16();
          case 2:
            return this.slice.read_uint32();
          case 3:
            return this.slice.len();
        }
      }).call(this);
    };

    PacketParser.prototype.parse_tag_and_len_new = function(c) {
      this.tag = c & 0x3f;
      return this.parse_tag_len_new();
    };

    PacketParser.prototype.parse_tag_len_new = function() {
      var a, c, go, i, lastlen, packet_length, segments;
      go = true;
      segments = [];
      this.len = 0;
      lastlen = 0;
      while (go) {
        go = false;
        c = this.slice.read_uint8();
        lastlen = c < 192 ? this.slice.read_uint8() : c === 255 ? this.slice.read_uint32() : c < 224 ? (a = (function() {
          var _i, _results;
          _results = [];
          for (i = _i = 0; _i < 2; i = ++_i) {
            _results.push(this.slice.read_uint8());
          }
          return _results;
        }).call(this), ((a[0] - 192) << 8) + (a[1] + 192)) : (this.header_len || (this.header_len = this.slice.offset()), packet_length = 1 << (c & 0x1f), segments.push(this.slice.read_buffer(packet_length)), go = true, packet_length);
        this.len += lastlen;
      }
      if (segments.length) {
        segments.push(this.slice.read_buffer(lastlen));
        this.body = new SlicerBuffer(Buffer.concat(segments));
        this.len = this.body.length;
        return this.real_packet_len = this.slice.offset();
      }
    };

    return PacketParser;

  })();

  exports.parse = parse = function(buf) {
    return (new MessageParser(new SlicerBuffer(buf))).parse();
  };

  fs = require('fs');

  (function(__iced_k) {
    __iced_deferrals = new iced.Deferrals(__iced_k, {
      filename: "src/packet/parser.iced"
    });
    fs.readFile('../../x', __iced_deferrals.defer({
      assign_fn: (function() {
        return function() {
          err = arguments[0];
          return res = arguments[1];
        };
      })(),
      lineno: 120
    }));
    __iced_deferrals._fulfill();
  })(function() {
    console.log(err);
    out = parse(res);
    console.log(inspect(out, {
      depth: null
    }));
    Processor = require('./processor').Processor;
    processor = new Processor(out);
    (function(__iced_k) {
      __iced_deferrals = new iced.Deferrals(__iced_k, {
        filename: "src/packet/parser.iced"
      });
      processor.verify_signatures(__iced_deferrals.defer({
        assign_fn: (function() {
          return function() {
            return err = arguments[0];
          };
        })(),
        lineno: 126
      }));
      __iced_deferrals._fulfill();
    })(function() {
      var _i, _len, _ref, _results, _while;
      console.log(err);
      _ref = out;
      _len = _ref.length;
      _i = 0;
      _results = [];
      _while = function(__iced_k) {
        var _break, _continue, _next;
        _break = function() {
          return __iced_k(_results);
        };
        _continue = function() {
          return iced.trampoline(function() {
            ++_i;
            return _while(__iced_k);
          });
        };
        _next = function(__iced_next_arg) {
          _results.push(__iced_next_arg);
          return _continue();
        };
        if (!(_i < _len)) {
          return _break();
        } else {
          p = _ref[_i];
          if (p.is_key_material()) {
            (function(__iced_k) {
              __iced_deferrals = new iced.Deferrals(__iced_k, {
                filename: "src/packet/parser.iced"
              });
              p.open({
                passphrase: 'asdfqwer'
              }, __iced_deferrals.defer({
                assign_fn: (function() {
                  return function() {
                    return err = arguments[0];
                  };
                })(),
                lineno: 129
              }));
              __iced_deferrals._fulfill();
            })(function() {
              console.log(err);
              err = p.key.sanity_check();
              return _next(console.log(err));
            });
          } else {
            return _continue();
          }
        }
      };
      _while(__iced_k);
    });
  });

}).call(this);
