...
(function() {
  var Base, Chain, Demux, Gets, InBlocker, Passthrough, ReverseAdapter, SimpleInit, StreamAdapter, assert, buf_index_of, bufcat, iced, make_esc, stream, __iced_k, __iced_k_noop, _ref,
  ...
  _ref = require('./util'), buf_index_of = _ref.buf_index_of, bufcat = _ref.bufcat;
            ...
            (function(__iced_k) {
              if (_this._dlen >= (pb = _this.peek_bytes())) {
                ...
                (function(__iced_k) {
                  ...
                  return __iced_k(_this._flowing = true);
                });
              } else {
                return __iced_k(eof != null ? (err = new Error("EOF before " + pb + " bytes"), data = null) : void 0);
              }
            })(__iced_k);
          } else {
            return __iced_k();
  ...
  Gets = (function(_super) {
    ...
    Gets.prototype.chunk = function(_arg, cb) {
      var buf, data, eof, err, front, i, newline, out, rest, retlen, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      data = _arg.data, eof = _arg.eof;
      out = err = null;
      newline = false;
      if ((data != null) && (i = buf_index_of(data, "\n".charCodeAt(0))) >= 0) {
        front = data.slice(0, index);
        this._buffers.push(front);
        out = Buffer.concat(this._buffers);
        rest = data.slice(index);
        this._buffers = rest.length ? [rest] : [];
        this._dlen = rest.length;
        newline = true;
      } else if ((data != null) || eof) {
        this._buffers.push(data);
        this._dlen += data.length;
        if (this._maxline && (this._dlen > this._maxline)) {
          buf = Buffer.concat(this._buffers);
          retlen = Math.floor(this._dlen / this._mod) * this._mod;
          out = buf.slice(0, retlen);
