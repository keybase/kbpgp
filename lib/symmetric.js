...
(function() {
  var AES, C, CAST5, SlicerBuffer, WordArray, allowed_ciphers, checksum2, ct, ecc_pkcs5_unpad_data, export_key_pgp, get_cipher, import_key_pgp, import_key_pgp_ct, triplesec, uint_to_buffer;
  ...
  exports.import_key_pgp_ct = import_key_pgp_ct = function(valid_msg, msg, random_key, pkcs5_padding) {
    var algo, c, checksum, cipher, err, found_cipher, i, key, offset, ret, selected_algo, selected_key, valid, _i, _j, _len, _ref;
    ...
    found_cipher = 0;
    for (_i = 0, _len = allowed_ciphers.length; _i < _len; _i++) {
      c = allowed_ciphers[_i];
      cipher = get_cipher(c);
      algo = ct.read_byte(msg, 0);
      valid = ct.normalize(valid_msg);
      valid &= ct.eq_int(algo, c);
      key = Buffer.alloc(cipher.key_size);
      for (i = _j = 0, _ref = cipher.key_size; 0 <= _ref ? _j < _ref : _j > _ref; i = 0 <= _ref ? ++_j : --_j) {
    ...
    key = selected_key.slice(0, cipher.key_size);
    return [!!found_cipher, new cipher.klass(WordArray.from_buffer(key))];
  };
