...
(function() {
  var C, K, Packet, Signature, eddsa, iced, konst, make_esc, unix_time, __iced_k, __iced_k_noop,
  ...
  Signature = (function(_super) {
    ...
    function Signature(_arg) {
      this.key = _arg.key, this.payload = _arg.payload, this.sig = _arg.sig, this.detached = _arg.detached, this.expire_in = _arg.expire_in;
    ...
    Signature.sign_payload = function(_arg) {
      var ctime, detached, etime, expire_in, payload;
      payload = _arg.payload, detached = _arg.detached, expire_in = _arg.expire_in, etime = _arg.etime;
      ctime = unix_time();
      if ((etime != null) && etime === 0) {
        etime = null;
      } else if (expire_in) {
        etime = ctime + expire_in;
      }
      return {
        payload: payload,
        detached: detached,
    ...
    Signature.box = function(_arg, cb) {
      var detached, esc, etime, expire_in, km, packet, pair, payload, sig, sp, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      km = _arg.km, payload = _arg.payload, expire_in = _arg.expire_in, etime = _arg.etime;
      esc = make_esc(cb, "@sign");
      pair = km.get_keypair();
      detached = true;
      sp = Signature.sign_payload({
        payload: payload,
        detached: detached,
        expire_in: expire_in,
        etime: etime
      ...
      (function(_this) {
        return (function(__iced_k) {
          ...
          pair.sign_kb(sp, esc(__iced_deferrals.defer({
            assign_fn: (function() {
              return function() {
