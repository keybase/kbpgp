...
(function() {
  ...
  KeyManager = (function() {
    ...
    KeyManager.prototype.fetch = function(keys, flags, cb) {
      var err, ff, flag, key, pki, _i, _len;
      err = key = null;
      pki = this.get_pgp_key_id();
      ff = 0;
      for (_i = 0, _len = flags.length; _i < _len; _i++) {
        flag = flags[_i];
        ff |= flag;
      }
      if (!(__indexOf.call(keys, pki) >= 0)) {
        err = new Error("No keys match the given fingerprint");
      } else if ((key = this.find_best_pgp_key(ff)) == null) {
