...
(function() {
  ...
  Engine = (function() {
    ...
    Engine.prototype._merge_private_subkey = function(k2, i) {
      var ekid, err, k;
      if (!this.key(k2).has_private()) {
        err = null;
      } else if ((ekid = this.ekid(k2)) == null) {
        err = new Error("Subkey " + i + " is malformed");
      } else if ((k = this._index[ekid]) == null) {
        err = new Error("Subkey " + i + " wasn't found in public key");
      } else if (this._merge_1_private(k, k2)) {
        console.log("successful merge private " + i);
        err = null;
      } else {
        err = new Error("subkey " + i + " can't be merged");
      }
      return err;
    };
